...
(* Lemma in_union: forall i A B,
  i `in` union A B <-> i `in` A \/ i `in` B.
Proof with auto.
  intros. split; intros.
  - unfold AtomSetImpl.In. inversion H. *)
...
Proof with auto.
  ...
  destruct (a==i)... 
    - discriminate H.
    - apply F.union_iff. destruct (Tlookup i A1)...
      -- left. eapply IHA1...
      -- right. eapply IHA2. apply H. 
Qed.
...
Proof with auto.
  ...
  apply AtomSetImpl.singleton_1 in H. rewrite H.
    - exists T. destruct (i==i)... exfalso. apply n...  (* fixme: why I need this? *)
    - apply AtomSetImpl.union_1 in H as [H1 | H2].
      -- apply IHT1 in H1. destruct H1 as [S H1]. rewrite H1. exists S...
      -- apply IHT2 in H2. destruct H2 as [S H2]. destruct (Tlookup i T1).
        --- exists t...
        --- rewrite H2. exists S...
Qed.  
...
Proof with auto.
  intros.
  induction H;try solve [inversion H0|inversion H1]; simpl in *...
  - inversion H3.
  - apply IHsub.
(*   
  - apply IHsub.
...
Fixpoint dropLabel (Z : atom) (T : typ) {struct T} : typ :=
  ...
  | typ_single l T => if (l==Z) then typ_top else typ_single l T
  | typ_and A B => typ_and (dropLabel Z A) (dropLabel Z B)
...
Proof with auto.
  ...
  subst.
  apply AtomSetProperties.union_subset_1.
  simpl. apply AtomSetProperties.union_subset_2.  (* fixme: where can I find them? *)
  apply AtomSetProperties.union_subset_3.
  - rewrite <- AtomSetProperties.union_assoc. eapply AtomSetProperties.subset_trans. apply IHA1. apply AtomSetProperties.union_subset_1.
  - eapply AtomSetProperties.subset_trans. apply IHA2. apply AtomSetProperties.union_subset_5. apply AtomSetProperties.union_subset_2.
Qed.
...
Proof with auto.
  ...
  destruct (a==i); simpl in *...
  - apply AtomSetProperties.subset_empty.
  - apply AtomSetProperties.subset_refl.
  - eapply KeySetProperties.subset_trans.
    -- apply AtomSetProperties.union_subset_4. apply IHA1.
    -- apply AtomSetProperties.union_subset_5. apply IHA2.
Qed.
...
Proof with auto.
  ...
  destruct H... *)
  - apply AtomSetImpl.singleton_1 in H. subst. destruct (i == i); simpl.
    -- simpl.
       (* hnf. intros. simpl in *. *)
       fsetdec.
    -- fsetdec.
  - apply AtomSetImpl.union_1 in H as [H1 | H2].
    -- apply IHA1 in H1. clear IHA1. clear IHA2. assert (collectLabel (dropLabel i A2) [<=] collectLabel A2). {
      apply drop_coolect_less.
    } fsetdec.
    -- apply IHA2 in H2. clear IHA1. clear IHA2. assert (collectLabel (dropLabel i A1) [<=] collectLabel A1). {
      apply drop_coolect_less.
    } fsetdec.
  (* destruct (a==i)...
  subst.
...
Proof with auto.
  ...
  destruct (a==i); subst; simpl...
  - apply notin_singleton_1 in H. exfalso...
  - apply AtomSetProperties.equal_refl.
  - apply notin_union in H as [H1 H2]. eapply AtomSetProperties.equal_trans.
    -- apply KeySetProperties.union_equal_2. apply IHA2...
    -- apply KeySetProperties.union_equal_1...
Qed.
...
Lemma Tlookup_drop_noeffect: forall i j A,
  i <> j ->
  Tlookup i (dropLabel j A) = Tlookup i A.
Proof with auto.
  intros. induction A;simpl in *;try solve [inversion H]...
  - destruct (a == j); subst; simpl.
    -- destruct (j == i)... exfalso...
    -- reflexivity.
...
Proof with auto.
  ...
  - remember (Tlookup j (dropLabel j A1)) as tl. destruct tl.
    -- symmetry in Heqtl. apply label_belong in Heqtl.
       assert (j `notin` collectLabel (dropLabel j A1)). apply notin_drop_self... exfalso...
    -- apply label_belong in H.
       assert (j `notin` collectLabel (dropLabel j A2)). apply notin_drop_self... exfalso...
  - remember (Tlookup j (dropLabel i A1)) as OA1. destruct OA1.
    -- apply IHA1 in H. rewrite H...
    -- apply IHA2 in H. assert (Heqdrop: Tlookup j (dropLabel i A1) = Tlookup j A1). {
      apply Tlookup_drop_noeffect...
    } rewrite <- Heqdrop. rewrite <- HeqOA1. assumption.
Qed.
...
Proof with auto.
  ...
  destruct (a==i)...
  destruct n...
  assert (Heq1: Tlookup i (dropLabel j A1) = Tlookup i A1). {
    apply Tlookup_drop_noeffect...
  } assert (Heq2: Tlookup i (dropLabel j A2) = Tlookup i A2). {
    apply Tlookup_drop_noeffect...
  } rewrite Heq1, Heq2 in *. apply H.
Qed.  
...
Proof with auto.
  ...
  - simpl in *. destruct (a==i)...
    + inversion H... intro x. intros...
    + inversion H.
  - simpl in *. destruct (Tlookup i A1).
    -- inversion H; subst. eapply AtomSetProperties.subset_trans. apply IHA1... apply AtomSetProperties.union_subset_1.
    -- eapply AtomSetProperties.subset_trans. apply IHA2... apply AtomSetProperties.union_subset_2.
Qed.
